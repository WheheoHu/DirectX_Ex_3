...
void Plane_Ring_Textrue::Render(IDirect3DDevice9 * _device)
{
	...
	_device->SetSamplerState(0, D3DSAMP_MAXANISOTROPY, 16);
...
Cylinder_Texture::Cylinder_Texture()
{
}
...
bool Cylinder_Texture::CreateBuffer(IDirect3DDevice9 * _device)
{
	HRESULT hr = 0;
	//创建顶点缓存
	hr = _device->CreateVertexBuffer(
...
void Cylinder_Texture::Render(IDirect3DDevice9 * _device)
{
	_device->SetStreamSource(0, _vb, 0, sizeof(TextureVertex));
	_device->SetIndices(_ib);
	_device->SetFVF(TextureVertex::FVF);
